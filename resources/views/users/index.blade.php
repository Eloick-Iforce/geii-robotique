            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                    <div x-data="{ search: '', users: JSON.parse('{{ json_encode($users) }}'), sortField: '', sortDirection: 'asc' }">
                        <div class="space-y-4">
                            <div class="flex flex-col sm:flex-row justify-between border-b border-gray-200 py-2">
                            <input x-model="search" type="text" placeholder="Rechercher par nom" class="mb-4 border-2 border-gray-300 bg-white h-10 px-5 pr-16 rounded-lg text-sm focus:outline-none">
                            ...
                            <template x-for="(user, index) in users.sort((a, b) => {
                                ...
                                <div x-show="!search || user.name.toLowerCase().includes(search.toLowerCase())" class="flex flex-col sm:flex-row justify-between border-b border-gray-200 py-2">
                                    <div class="px-4 py-2" x-text="user.name"></div>
                                    <div class="px-4 py-2" x-text="user.email"></div>
